...
class ModeManager:
    ...
    def return_to_menu(self):
        """Leave the active module and return to the top-level menu."""
        if self.active_mode:
        ...
        if self.state_machine:
            self.state_machine.state = "MENU"
            self.state_machine.menu_index = 0
            self.state_machine.palm_start_time = None
    ...
    def handle_command(self, command):
        ...
        elif command == "RESET" and self.active_mode == "drawing":
            self.return_to_menu()
            return
