...
def apply_startup_mode():
    """Apply an optional startup mode from the launcher or command line."""
    start_mode = os.environ.get("GESTUREHUB_START_MODE")
    if not start_mode and len(sys.argv) > 1:
        start_mode = sys.argv[1]

    if not start_mode:
        return

    normalized_mode = start_mode.lower().strip()
    if normalized_mode in ("menu", "full", "default"):
        return

    if normalized_mode == "music":
        mode_manager.switch("music")
        music_module = mode_manager.get_module("music")
        if music_module:
            music_module.set_mode("local")
    elif normalized_mode in mode_manager.modules:
        mode_manager.switch(normalized_mode)
    else:
        print(f"WARNING: Unknown startup mode '{start_mode}'. Starting in menu mode.")
        return

    state_machine.state = "IDLE"
    state_machine.palm_start_time = None
    state_machine.menu_index = 0
    print(f"Startup mode applied: {normalized_mode}")
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            # Convert MediaPipe landmarks to normalized coordinates [x, y, z]
            landmarks = []
            ...
            # hand_pos is in pixel coordinates for motion detection
            hand_pos = (int(landmarks[0][0] * w), int(landmarks[0][1] * h))

            stable_gesture = gesture_controller.stabilize(gesture)
            commands = gesture_controller.detect_commands(
                stable_gesture, hand_pos, landmarks, active_mode=mode_manager.get_active_mode()
            )

            # Debug output
            if DEBUG_GESTURES and (gesture != "UNKNOWN" or stable_gesture != "NO_HAND"):
                print(f"DEBUG: fingers={fingers}, gesture='{gesture}', stable='{stable_gesture}', commands={commands}")
    ...
    if current_state == "MENU":
        ...
        elif active_mode == "fruit":
            hl = results.multi_hand_landmarks[0] if results.multi_hand_landmarks else None
            frame = fruit.update(frame, hl)
        else:
